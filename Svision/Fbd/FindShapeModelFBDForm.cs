...
namespace Svision
{
    public partial class FindShapeModelFBDForm : Form
    {
        ...
        private findShapeModelClass fSMC = new findShapeModelClass();//参数类
        private HTuple templateModel;
        private double[] rows, columns, angles, scores;
        int[] templateID;
        ...
        private createPara cPara = new createPara();
        public struct findParas
        {
            public double[] angleStart;
        ...
        private findParas fParas = new findParas();
        public findParas wFparas = new findParas();
        private findParas fParasone = new findParas();
        private findPara fPara = new findPara();
        public FindShapeModelFBDForm(int idx)
        {
            InitializeComponent();
        ...
        private void WFparaInit()
        {
            ...
            {       
                wFparas.angleStart[i] = UserCode.GetInstance().gProCd[currentIndex].gSSP.angleStart[i];
                wFparas.angleExtent[i] = UserCode.GetInstance().gProCd[currentIndex].gSSP.angleExtent[i];
                wFparas.minScore[i] = UserCode.GetInstance().gProCd[currentIndex].gSSP.minScore[i];
                wFparas.numMatches[i] = UserCode.GetInstance().gProCd[currentIndex].gSSP.numMatches[i];
                wFparas.subPixel[i * 2] = UserCode.GetInstance().gProCd[currentIndex].gSSP.subPixel[i*2];
                wFparas.subPixel[i * 2 + 1] = UserCode.GetInstance().gProCd[currentIndex].gSSP.subPixel[i * 2 + 1];
                wFparas.numLevels[i * 2] = UserCode.GetInstance().gProCd[currentIndex].gSSP.numLevels[i * 2];
                wFparas.numLevels[i * 2 + 1] = UserCode.GetInstance().gProCd[currentIndex].gSSP.numLevels[i * 2 + 1];
                wFparas.greediness[i] = UserCode.GetInstance().gProCd[currentIndex].gSSP.greediness[i];
            }
            wFparas.maxOverlap = UserCode.GetInstance().gProCd[currentIndex].gSSP.maxOverlap;

            if (UserCode.GetInstance().gProCd[currentIndex].gSSP.shapeModel == null)
            {
            ...
            else
            {
                ...
                templateNumber = UserCode.GetInstance().gProCd[currentIndex].gSSP.shapeModel.TupleLength();
                tModelIDs = UserCode.GetInstance().gProCd[currentIndex].gSSP.shapeModel;
                templateModelRegions = UserCode.GetInstance().gProCd[currentIndex].gSSP.shapeModelRegion;
                modelPoints.Clear();
                if (UserCode.GetInstance().gProCd[currentIndex].gSSP.shapeModelPoints!=null)
                {
                    for (int io = 0; io < UserCode.GetInstance().gProCd[currentIndex].gSSP.shapeModelPoints.Count;io++ )
                    {
                        PointF pf = new PointF(0,0);
                        pf.X = UserCode.GetInstance().gProCd[currentIndex].gSSP.shapeModelPoints[io].X;
                        pf.Y = UserCode.GetInstance().gProCd[currentIndex].gSSP.shapeModelPoints[io].Y;
                        modelPoints.Add(pf);
                    }
                }
                ...
                templateModelImages = UserCode.GetInstance().gProCd[currentIndex].gSSP.shapeModelImage;
                // checkBoxMultiplePara.Checked = UserCode.GetInstance().gProCd[currentIndex].boolData[1];
                checkBoxBorderShapeModel.Checked = UserCode.GetInstance().gProCd[currentIndex].gSSP.isBorderShapeModelChecked;

                if (UserCode.GetInstance().gProCd[currentIndex].gSSP.shapeModel.TupleLength() == 1)         //单模板
                {
                    checkedListBoxTemplate.Items.Add("模版1", false);
                    if (UserCode.GetInstance().gProCd[currentIndex].gSSP.modelIsChecked[0])
                    {
                        checkedListBoxTemplate.SetItemChecked(0, true);
                        numericUpDownTestStartAngle.Value = (decimal)UserCode.GetInstance().gProCd[currentIndex].gSSP.angleStart[MAX_MODEL_MUL_PARA_NUM-1];
                        numericUpDownTestExternAngle.Value = (decimal)UserCode.GetInstance().gProCd[currentIndex].gSSP.angleExtent[MAX_MODEL_MUL_PARA_NUM - 1];
                        numericUpDownTestMinScore.Value = (decimal)UserCode.GetInstance().gProCd[currentIndex].gSSP.minScore[MAX_MODEL_MUL_PARA_NUM - 1];
                        numericUpDownTestNumber.Value = (decimal)UserCode.GetInstance().gProCd[currentIndex].gSSP.numMatches[MAX_MODEL_MUL_PARA_NUM - 1];
                        numericUpDownTestOverLap.Value = (decimal)UserCode.GetInstance().gProCd[currentIndex].gSSP.maxOverlap;
                        comboBoxSubPixel1.SelectedIndex = (int)UserCode.GetInstance().gProCd[currentIndex].gSSP.subPixel[(MAX_MODEL_MUL_PARA_NUM - 1)*2];
                        comboBoxSubPixel2.SelectedIndex = (int)UserCode.GetInstance().gProCd[currentIndex].gSSP.subPixel[(MAX_MODEL_MUL_PARA_NUM - 1) * 2+1];
                        numericUpDownTestGreediness.Value = (decimal)UserCode.GetInstance().gProCd[currentIndex].gSSP.greediness[MAX_MODEL_MUL_PARA_NUM - 1];
                        if (UserCode.GetInstance().gProCd[currentIndex].gSSP.numLevels[(MAX_MODEL_MUL_PARA_NUM - 1) * 2] == 0 && UserCode.GetInstance().gProCd[currentIndex].gSSP.numLevels[(MAX_MODEL_MUL_PARA_NUM - 1) * 2+1] == 0)
                        {
                        ...
                        else
                        {
                            ...
                            numericUpDownTestNumberLevelsLarge.Value = (decimal)UserCode.GetInstance().gProCd[currentIndex].gSSP.numLevels[(MAX_MODEL_MUL_PARA_NUM - 1) * 2];
                            numericUpDownTestNumberLevelsSmall.Value = (decimal)UserCode.GetInstance().gProCd[currentIndex].gSSP.numLevels[(MAX_MODEL_MUL_PARA_NUM - 1) * 2+1];
                        }
                ...
                {
                    int i_num = 0;
                    for (int i = 0; i < UserCode.GetInstance().gProCd[currentIndex].gSSP.shapeModel.TupleLength(); i++)
                    {
                        checkedListBoxTemplate.Items.Add("模版" + (i + 1).ToString(), false);
                        if (UserCode.GetInstance().gProCd[currentIndex].gSSP.modelIsChecked[i])
                        {
                    ...
                    if (i_num!=0)
                    {
                        if (UserCode.GetInstance().gProCd[currentIndex].gSSP.isMultiplePara)//多模版多参数
                        {
                            checkBoxModelIsOK.Checked = true;
            ...
            arrangeIndex = UserCode.GetInstance().gProCd[currentIndex].gSSP.arrangeIndex;
            switch (arrangeIndex)
            {
        ...
        private void FindShapeModelFBDForm_Load(object sender, EventArgs e)
        {
            ...
            cPara.angleStep = 0;
            cPara.optimization = new int[2] { 0, 1 };
            ...
            comboBoxSubPixel2.SelectedIndex = 1;
            WFparaInit();
        }
        ...
        private void buttonInspectShapeModel_Click(object sender, EventArgs e)
        {
            if (image != null && imageRealRegion != null)
            {
                try
                {
                    ...
                    fSMC.setCreatShapeModelPara(cPara.numberLevel, cPara.angleStart, cPara.angleExtent, cPara.angleStep, cPara.optimization, cPara.metric, cPara.contrast, cPara.minContrast);
        ...
        private void ButtonCreateModel_Click(object sender, EventArgs e)
        {
            if (image != null && imageRealRegion != null)
            {
                ...
                try
                {
                    ...
                    fSMC.createShapeModel(imgtemp, out tModelID);
                    HTuple ra, rcr, rcc;
                    ...
                    temp.Dispose();
                    imgtemp.Dispose();
                    UserCode.GetInstance().isOverFlag[currentIndex] = 11;
                }                
                catch (System.Exception ex)
                {
                ...
                try
                {
                    if (checkBoxModelIsOK.Checked == true)
                    {
                        if (checkBoxMultiplePara.Checked == true && templateModel.TupleLength() != 1)
                        {
                            ...
                            fSMC.setFindShapeModelsPara((int)templateModel.TupleLength(), fParas.angleStart, fParas.angleExtent, fParas.minScore, fParas.numMatches, fParas.maxOverlap, fParas.subPixel, fParas.numLevels, fParas.greediness);//Set findShapeModels parameters.
                            ...
                            fSMC.findShapeModels(templateModel, image,arrangeIndex, out rows, out columns, out angles, out scores, out templateID);// Find the Multiple models without display the results.
                            HOperatorSet.CountSeconds(out seconds2);
                            //UserCode.GetInstance().isOverFlag[currentIndex] = 21;
                            if (rows.Length != 0)
                            {
                                ...
                                {
                                    HTuple hvHomMat2D;
                                    HOperatorSet.VectorAngleToRigid(templateModelPoints[templateID[i]].Y, templateModelPoints[templateID[i]].X, 0, rows[i], columns[i], angles[i], out hvHomMat2D);
                        ...
                        else if (checkBoxMultiplePara.Checked == false && templateModel.TupleLength() != 1)
                        {
                            ...
                            fSMC.setFindShapeModelsPara((int)templateModel.TupleLength(), fParasone.angleStart, fParasone.angleExtent, fParasone.minScore, fParasone.numMatches, fParasone.maxOverlap, fParasone.subPixel, fParasone.numLevels, fParasone.greediness);//Set findShapeModels parameters.
                            //fsmc.findShapeModels(hoTestImage,hvWindowHandle, 1, out rows, out columns, out angles, out scores,out templateID);// Find the Multiple models and display the results.
                            fSMC.findShapeModels(templateModel, image,arrangeIndex, out rows, out columns, out angles, out scores, out templateID);// Find the Multiple models without display the results.
                            HOperatorSet.CountSeconds(out seconds2);
                            if (rows.Length != 0)
                        ...
                        else if (templateModel.TupleLength() == 1)
                        {
                            ...
                            fSMC.setFindShapeModelPara(fPara.angleStart, fPara.angleExtent, fPara.minScore, fPara.numMatches, fPara.maxOverlap, fPara.subPixel, fPara.numLevels, fPara.greediness);//Set findShapeModels parameters.
                            //fsmc.findShapeModels(hoTestImage,hvWindowHandle, 1, out rows, out columns, out angles, out scores,out templateID);// Find the Multiple models and display the results.
                            fSMC.findShapeModel(image, templateModel,arrangeIndex, out rows, out columns, out angles, out scores);// Find the Multiple models without display the results.
                            HOperatorSet.CountSeconds(out seconds2);
                            if (rows.Length != 0)
                            {
                                HObject tempContour, tempRegion, tempCross, imageTempShowWithOutPoint;
                                ...
                                {
                                    HTuple hvHomMat2D;
                                    HOperatorSet.VectorAngleToRigid(templateModelPoints[0].Y, templateModelPoints[0].X, 0, rows[i], columns[i], angles[i], out hvHomMat2D);
        ...
        private void buttonShapeSearchConfirm_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                UserCode.GetInstance().gProCd[currentIndex].gSSP.arrangeIndex = arrangeIndex;
                UserCode.GetInstance().gProCd[currentIndex].gSSP.isBorderShapeModelChecked = checkBoxBorderShapeModel.Checked;
                UserCode.GetInstance().gProCd[currentIndex].gSSP.isMultiplePara = checkBoxMultiplePara.Checked;
                UserCode.GetInstance().gProCd[currentIndex].gSSP.shapeModel = tModelIDs;
                UserCode.GetInstance().gProCd[currentIndex].gSSP.shapeModelImage = templateModelImages;
                UserCode.GetInstance().gProCd[currentIndex].gSSP.shapeModelRegion = templateModelRegions;
                UserCode.GetInstance().gProCd[currentIndex].gSSP.shapeModelPoints.Clear();
                if (modelPoints != null)
                {
                    for (int io = 0; io < modelPoints.Count; io++)
                    {
                        ...
                        UserCode.GetInstance().gProCd[currentIndex].gSSP.shapeModelPoints.Add(pf);
                    }
                }

                int Max_Object_Num_Temp = UserCode.GetInstance().gProCd[currentIndex].gSSP.Max_Object_Num;
                UserCode.GetInstance().gProCd[currentIndex].gSSP.Max_Object_Num = (int)numericUpDownTestNumber.Value;
                for (int i = 0; i < tModelIDs.TupleLength(); i++)
                {
                    if (modelIfCkecked[i])
                    {
                        UserCode.GetInstance().gProCd[currentIndex].gSSP.modelIsChecked[i] = true;
                    }
                    else
                        UserCode.GetInstance().gProCd[currentIndex].gSSP.modelIsChecked[i] = false;
                ...
                if (templateModel != null)
                {
                    ...
                    {
                        UserCode.GetInstance().gProCd[currentIndex].gSSP.angleStart[MAX_MODEL_MUL_PARA_NUM - 1] = fPara.angleStart;
                        UserCode.GetInstance().gProCd[currentIndex].gSSP.angleExtent[MAX_MODEL_MUL_PARA_NUM - 1] = fPara.angleExtent;
                        UserCode.GetInstance().gProCd[currentIndex].gSSP.minScore[MAX_MODEL_MUL_PARA_NUM - 1] = fPara.minScore;
                        UserCode.GetInstance().gProCd[currentIndex].gSSP.numMatches[MAX_MODEL_MUL_PARA_NUM - 1] = fPara.numMatches;
                        UserCode.GetInstance().gProCd[currentIndex].gSSP.subPixel[(MAX_MODEL_MUL_PARA_NUM - 1) * 2] = fPara.subPixel[0];
                        UserCode.GetInstance().gProCd[currentIndex].gSSP.subPixel[(MAX_MODEL_MUL_PARA_NUM - 1) * 2+1] = fPara.subPixel[1];
                        UserCode.GetInstance().gProCd[currentIndex].gSSP.numLevels[(MAX_MODEL_MUL_PARA_NUM - 1) * 2] = fPara.numLevels[0];
                        UserCode.GetInstance().gProCd[currentIndex].gSSP.numLevels[(MAX_MODEL_MUL_PARA_NUM - 1) * 2+1] = fPara.numLevels[1];
                        UserCode.GetInstance().gProCd[currentIndex].gSSP.greediness[MAX_MODEL_MUL_PARA_NUM - 1] = fPara.greediness;
                        UserCode.GetInstance().gProCd[currentIndex].gSSP.maxOverlap = fPara.maxOverlap;
                        UserCode.GetInstance().gProCd[currentIndex].gSSP.Max_Object_Num = fPara.numMatches;
                    ...
                    else                                                //多模板;
                    {
                        ...
                        {
                            UserCode.GetInstance().gProCd[currentIndex].gSSP.maxOverlap = wFparas.maxOverlap;
                            for (int i = 0; i < tModelIDs.TupleLength(); i++)
                            {
                                UserCode.GetInstance().gProCd[currentIndex].gSSP.angleStart[i] = wFparas.angleStart[i];
                                UserCode.GetInstance().gProCd[currentIndex].gSSP.angleExtent[i] = wFparas.angleExtent[i];
                                UserCode.GetInstance().gProCd[currentIndex].gSSP.minScore[i] = wFparas.minScore[i];
                                UserCode.GetInstance().gProCd[currentIndex].gSSP.numMatches[i] = wFparas.numMatches[i];
                                UserCode.GetInstance().gProCd[currentIndex].gSSP.subPixel[2 * i] = wFparas.subPixel[i];
                                UserCode.GetInstance().gProCd[currentIndex].gSSP.subPixel[2 * i + 1] = wFparas.subPixel[2 * i + 1];
                                UserCode.GetInstance().gProCd[currentIndex].gSSP.numLevels[2 * i] = wFparas.numLevels[2 * i];
                                UserCode.GetInstance().gProCd[currentIndex].gSSP.numLevels[2 * i + 1] = wFparas.numLevels[2 * i + 1];
                                UserCode.GetInstance().gProCd[currentIndex].gSSP.greediness[i] = wFparas.greediness[i];
                                
                            }
                            UserCode.GetInstance().gProCd[currentIndex].gSSP.isMultiplePara = true;
                            UserCode.GetInstance().gProCd[currentIndex].gSSP.Max_Object_Num = wFparas.numMatches.Sum();
                        }
                        else
                        {
                            UserCode.GetInstance().gProCd[currentIndex].gSSP.angleStart[MAX_MODEL_MUL_PARA_NUM - 1] = fParasone.angleStart[0];
                            UserCode.GetInstance().gProCd[currentIndex].gSSP.angleExtent[MAX_MODEL_MUL_PARA_NUM - 1] = fParasone.angleExtent[0];
                            UserCode.GetInstance().gProCd[currentIndex].gSSP.minScore[MAX_MODEL_MUL_PARA_NUM - 1] = fParasone.minScore[0];
                            UserCode.GetInstance().gProCd[currentIndex].gSSP.numMatches[MAX_MODEL_MUL_PARA_NUM - 1] = fParasone.numMatches[0];
                            UserCode.GetInstance().gProCd[currentIndex].gSSP.subPixel[(MAX_MODEL_MUL_PARA_NUM - 1) * 2] = fParasone.subPixel[0];
                            UserCode.GetInstance().gProCd[currentIndex].gSSP.subPixel[(MAX_MODEL_MUL_PARA_NUM - 1) * 2 + 1] = fParasone.subPixel[1];
                            UserCode.GetInstance().gProCd[currentIndex].gSSP.numLevels[(MAX_MODEL_MUL_PARA_NUM - 1) * 2] = fParasone.numLevels[0];
                            UserCode.GetInstance().gProCd[currentIndex].gSSP.numLevels[(MAX_MODEL_MUL_PARA_NUM - 1) * 2 + 1] = fParasone.numLevels[1];
                            UserCode.GetInstance().gProCd[currentIndex].gSSP.greediness[MAX_MODEL_MUL_PARA_NUM - 1] = fParasone.greediness[0];
                            UserCode.GetInstance().gProCd[currentIndex].gSSP.maxOverlap = fParasone.maxOverlap;
                            UserCode.GetInstance().gProCd[currentIndex].gSSP.isMultiplePara = false;
                            UserCode.GetInstance().gProCd[currentIndex].gSSP.Max_Object_Num = fParasone.numMatches[0];
                        }
                ...
                UserCode.GetInstance().gProCd[currentIndex].gSSP.outPutStringList.Clear();
                UserCode.GetInstance().gProCd[currentIndex].gSSP.outPutStringList.Add("识别目标个数");
                for (int i=0;i<UserCode.GetInstance().gProCd[currentIndex].gSSP.Max_Object_Num;i++)
                {

                    UserCode.GetInstance().gProCd[currentIndex].gSSP.outPutStringList.Add("第" + i.ToString() + "个目标X坐标");
                    UserCode.GetInstance().gProCd[currentIndex].gSSP.outPutStringList.Add("第" + i.ToString() + "个目标Y坐标");
                    UserCode.GetInstance().gProCd[currentIndex].gSSP.outPutStringList.Add("第" + i.ToString() + "个目标种类");
                    UserCode.GetInstance().gProCd[currentIndex].gSSP.outPutStringList.Add("第" + i.ToString() + "个目标角度");
                    UserCode.GetInstance().gProCd[currentIndex].gSSP.outPutStringList.Add("第" + i.ToString() + "个目标得分");        

                }

                if (Max_Object_Num_Temp != UserCode.GetInstance().gProCd[currentIndex].gSSP.Max_Object_Num)
                {
                    for (int i = 0; i < 20;i++ )
                    {
                        if (UserCode.GetInstance().isOverFlag[i]==32)
                        {
                            if ((ProCodeCls.MainFunction)UserCode.GetInstance().gProCd[i].gSOP.sendDataInfoList[0].funcID == ProCodeCls.MainFunction.MeasureShapeSearchFBD
        ...
        private void textCommand2_Execute(DevExpress.CodeRush.Core.ExecuteTextCommandEventArgs ea)
        {
