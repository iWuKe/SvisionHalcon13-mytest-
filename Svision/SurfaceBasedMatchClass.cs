...
namespace Svision
{
    class SurfaceBasedMatchClass
    {
        HTuple sfm;
       /// <summary>
       /// createSurfaceModel参数结构体
       /// </summary>
        private struct createSurfaceModelPara
        {
            public HTuple RelSamplingDist;
            public HTuple GenParamName;
            public HTuple GenParamValue;
            
            public createSurfaceModelPara(bool CreateSurfaceModelParaDefault, float RelSamplingDistanceTemp, bool model_invert_normalsTemp, float pose_ref_rel_sampling_distanceTemp, float feat_step_size_relTemp, int feat_angle_resolutionTemp, bool train_3d_edgesTemp)//相当于构造函数
            {
                
                RelSamplingDist = null;
                GenParamName = null;
                GenParamValue = null;
                //设置采样距离参数
                RelSamplingDist = RelSamplingDistanceTemp;
                string model_invert_normals = model_invert_normalsTemp.ToString().ToLower();
                string train_3d_edges = train_3d_edgesTemp.ToString().ToLower();

                //设置生成模型参数对GenParamName和GenParamValue
                //GenParamName设定参数的名称，GenParamValue设置该参数对应的值。该值可能是bool类型，也可能是数值类型，与对应参数名有关。
                
                //参数列表如下：
                //model_invert_normals：
                    //该参数能够反转模型曲面法线方向。当三维模型是通过CAD文件导入时，则法线有可能指向相反的方向。如果遇在
                    //场景表面的“外侧”上找到模型的效果，并且该模型是通过CAD文件创建的，此时将该参数设置为true。
                    //Possible values: 'false', 'true'
                    //Default value:'false'
                //pose_ref_rel_sampling_distance：
                    //设置相对于对象直径的姿态细化的采样距离。减小这个值可以导致更准确的姿态细化，但会导致更大的模型和更慢
                    //的模型生成和细化。增加价值会导致姿态细化准确率降低，但得到模型更小更快。默认值：0.01。
                    //建议值：0.05, 0.02, 0.01, 0.005
                //feat_step_size_rel：
                    //默认设置成点云采样距离即RelSamplingDistance参数值，不建议修改。当场景中噪声很强时，可以提高该值来
                    //提高匹配的鲁棒性。
                //feat_angle_resolution:
                    //该参数设置点对的离散化方向，默认值为30，不建议修改。提高该值会增加匹配的精度，但是会降低抵抗错误法线
                    //方向的鲁棒性。当场景中噪声点特别多的时候，可以适当降低该值，设置为20或25，来提高抵抗错误法线方向的
                    //鲁棒性。
                //train_3d_edges：
                    //该参数设置模板匹配是否支持边缘，默认设置为false，支持边缘时需要提供mesh类型的三维模型，比如triangle
                    //mesh或者polygon mesh
                //SurfaceModelID
                    //生成的曲面模型
                ...
                //使用默认参数
                if (CreateSurfaceModelParaDefault == true)
                {
                ...
                {
                    GenParamName = ((HTuple)("model_invert_normals")).TupleConcat("model_invert_normals").TupleConcat("pose_ref_rel_sampling_distance").TupleConcat("feat_step_size_relTemp").TupleConcat("feat_angle_resolution").TupleConcat("train_3d_edges");
                    GenParamValue = ((HTuple)model_invert_normals).TupleConcat(pose_ref_rel_sampling_distanceTemp).TupleConcat(feat_step_size_relTemp).TupleConcat(feat_angle_resolutionTemp).TupleConcat(train_3d_edges);
        ...
        private createSurfaceModelPara createSurfaceModelP;
        //public void setCreateSurfaceModelPara(createSurfaceModelParaDefault,float RelSamplingDistanceTemp, bool model_invert_normalsTemp, float pose_ref_rel_sampling_distanceTemp, float feat_step_size_relTemp, int feat_angle_resolutionTemp, bool train_3d_edgesTemp)
        //{
        //    createSurfaceModelP = new createSurfaceModelPara(RelSamplingDistanceTemp, model_invert_normalsTemp,pose_ref_rel_sampling_distanceTemp,feat_step_size_relTemp, feat_angle_resolutionTemp,train_3d_edgesTemp);

        //}

        public void createSurfaceModel(HTuple objModel3D, out HTuple SFMID)//按照参数设置创建模板
        {
            HTuple poseout;
            try
            {
                HOperatorSet.CreateSurfaceModel(objModel3D, createSurfaceModelP.RelSamplingDist, createSurfaceModelP.GenParamName, createSurfaceModelP.GenParamValue, out SFMID);
                VisualizeObjModel3d visual3d = new VisualizeObjModel3d();
                //visual3d.visualize_object_model_3d(SBMWindowHandle, objModel3D, (new HTuple()), (new HTuple()), (new HTuple()), (new HTuple()), (new HTuple("表面模型")), (new HTuple()), (new HTuple()), out PoseOut);
        ...
        /// 保存模板
        /// </summary>
        public void saveSurfaceModel(string fileName, string filePath, HTuple SFMID)
        {
            string fullFileName = filePath + fileName + ".shm";
            HOperatorSet.WriteSurfaceModel(SFMID, (HTuple)fullFileName);
        ...
        /// 清空模板
        /// </summary>
        /// <param name="SFMID"></param>
        public void clearSurfaceModel(HTuple SFMID)
        {
            HOperatorSet.ClearSurfaceModel(SFMID);
        ...
        public void readSurfaceModel()
        {
            ...
            {
                HOperatorSet.ReadSurfaceModel(dlg.FileName, out sfm);
